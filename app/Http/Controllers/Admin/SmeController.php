...
class SmeController extends Controller
{
    ...
    {
         $datas = Admin::where('role_id',2)->orderBy('id', 'desc')->get();
         ...
         return Datatables::of($datas)
                            ...
                            '  <a href="'.route('sme.status.update',['id1' => $data->id, 'id2' => 0]).'" 
                            class="text-13 py-2 px-8 bg-success-50 text-success-600 d-inline-flex align-items-center gap-8 rounded-pill"
                                ...
                                :
                                '<a href="'.route('sme.status.update',['id1' => $data->id, 'id2' => 1]).'"  
                            ...
                            ->addColumn('action', function(Admin $data) {
                                return '<a href="'.route('sme.edit',$data->id).'" class="bg-main-50 text-main-600 py-2 px-14 rounded-pill hover-bg-main-600 hover-text-white">Edit</a>
                                <a href="'.route('sme.changepassword',$data->id).'" class="bg-main-50 text-main-600 py-2 px-14 rounded-pill hover-bg-main-600 hover-text-white">Change Password</a>';
    ...
    {
        $lobs = Lob::all();
        return view('admin.sme.create',compact('lobs'));
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->back()->with('success','SME create Successfully');    
    ...
    public function edit($id)
    {
        try {
            ...
            return view('admin.sme.edit', compact('sme','lobs'));  
        } catch (\Illuminate\Database\Eloquent\ModelNotFoundException $e) {
            return redirect()->route('sme')->with('error', 'SME not found');
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->back()->with('success','SME update Successfully');
    }
    ...
    public function changepassword($id)
    {
        try {
            $sme = Admin::findOrFail($id);
            return view('admin.sme.changepassword', compact('sme'));  
        } catch (\Illuminate\Database\Eloquent\ModelNotFoundException $e) {
            return redirect()->route('sme')->with('error', 'sme not found');
        } catch (\Illuminate\Database\QueryException $e) {
            abort(500);
